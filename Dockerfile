...
ADD postboost-cli /usr/bin/postboost-cli
ADD _postboost-cli /usr/local/share/zsh/site-functions/_postboost-cli
ADD postboost-cli.bash-completion /etc/bash-completion.d/postboost-cli
RUN chmod 755 /usr/bin/postboost-cli
...
RUN echo '\n\
. /etc/bash_completion\n\
source /etc/bash-completion.d/postboost-cli\n\
' >> ~/.bashrc

#
# Setup prompt
#
RUN echo 'export PS1="[PostBoost API] \$ "' >> ~/.bashrc
RUN echo 'export PROMPT="[PostBoost API] \$ "' >> ~/.zshrc
...
line REST client for $(tput setaf 6)PostBoost API$(tput sgr0).\n\
\n\
