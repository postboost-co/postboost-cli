...
This is a Bash client script for accessing PostBoost API service.
...
$ ./postboost-cli <operationId> -h

# Make GET request
./postboost-cli --host http://<hostname>:<port> --accept xml <operationId> <queryParam1>=<value1> <header_key1>:<header_value2>

# Make GET request using arbitrary curl options (must be passed before <operationId>) to an SSL service using username:password
postboost-cli -k -sS --tlsv1.2 --host https://<hostname> -u <user>:<password> --accept xml <operationId> <queryParam1>=<value1> <header_key1>:<header_value2>

# Make POST request
$ echo '<body_content>' | postboost-cli --host <hostname> --content-type json <operationId> -
...
#   "key3": 23
# }
$ echo '<body_content>' | postboost-cli --host <hostname> --content-type json <operationId> key1==value1 key2=value2 key3:=23 -

# Make POST request with form data
$ postboost-cli --host <hostname> <operationId> key1:=value1 key2:=value2 key3:=23

# Preview the cURL command without actually executing it
$ postboost-cli --host http://<hostname>:<port> --dry-run <operationid>
...
```shell
source postboost-cli.bash-completion
```

Alternatively, the script can be copied to the `/etc/bash-completion.d` (or on OSX with Homebrew to `/usr/local/etc/bash-completion.d`):

```shell
sudo cp postboost-cli.bash-completion /etc/bash-completion.d/postboost-cli
```
...
In Zsh, the generated `_postboost-cli` Zsh completion file must be copied to one of the folders under `$FPATH` variable.
